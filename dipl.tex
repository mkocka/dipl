...
From such results is eminent that there is something wrong with the WDs interior. As the ionization 
is increasing by higher temperatures, the bigger pressure otherwise helps recombination. However 
...
occupied, this kind of fermion gas is called completely degenerate. 
...
\end{equation}
where Nxyz are integer quantum numbers for each dimension. The momentum can be written 
...
\begin{equation}
 N_e = 2\left ( \frac{1}{8} \right )\left ( \frac{4}{3} a\pi N^3\right ) \Rightarrow N = \left ( \frac{3N_e}{\pi} \right )^{1/3} .
\end{equation}
...
\section{Degenerate electrons \& pressure}
