...
\chapter{Masses of white dwarfs in intermediate polars}
 As it was mention before, the mass of the WD is most fundamental parameter in the CVs. It is also the 
main parameter characterizing accretion flow, the emission from accretion region and mainly it is the 
parameter which governs the dynamics of orbital motion of the system. The knowledge of WDs masses in 
the CVs plays a fundamental role in understanding the binary evolution. 

Previous attempts to determine the WDs masses of IPs using X-ray measurements did not succeed well, 
because data from different instruments led to different mass estimations. In the all cases, the 
masses  determined by X-ray measurements where substantially higher then masses derived from optical 
observations. The best example is XY Ari, which is the eclipsing IP with very small uncertainty in 
...
by other system components such as the accretion disc or the main-sequence star and it avoids 
complications due to cold or warm absorbents as well as fluorescence emission (\citet{2009A&A...496..121B}). 
The big disadvantages of this method is the weakness of the X-ray flux. In spite of the fact that the IPs 
are one of the brighter sources in hard X-ray sky, their photon count is relatively low. Thus a sensitive 
instrument must be used for this kind of observation. There are three instruments up to date which are 
widely use for this kind of observations. The Swift/NASA Burst Alert Telescope, the Hard X-ray Detector
(HXD) on-board Suzaku/JAXA space mission and the IBIS on-board INTEGRAL/ESA.

The idea how to study the post-shock region and the X-ray emission from the IPs and then 
determine the mass of WD is then try to cover broad bands from soft to hard X-rays. Ideally 
from 1keV up to 100 keV. In the softer X-rays below 10 keV are however several others features 
presents, but the photons flux is much more larger than in over 15 keV bands. 

In this case, it is necessary to combine two different detectors, one for soft and other 
for hard X-ray. The good combination could be XMM-Newton for soft X-ray in range 1–10 keV 
and INTEGRAL for hard X-ray from 20–100 keV. 

This combination could be a very good compromise. The photon flux in the soft X-ray is high enough 
to allows shorts pointings of XMM with good s/n ratio. However that fitting model for such spectrum 
can be complicated because of presence of several other features e.g. reflections or the 6.4 keV 
emission line.

The hard X-rays are important for location of spectral energy cut-off and therefore for good fit 
of the model, either power-law or bremsstrahlung. The INTEGRAL/IBIS is due to relatively good 
sesitivity and large field of view (FOV) suitable instrument. The large FOV allows good cover 
of X-ray sky leading to long exposures which are needed in this band. 
